...
contract ssi_contract is ERC721URIStorage, Ownable {
    ...
    struct TokenDetails {
        string tokenURI;
        address owner;
        uint256 expirationTime; // Add expiration time field
    }

    mapping(uint256 => TokenDetails) private tokenDetails;
    mapping(address => uint256[]) private tokensByOwner;

    function mint(address useraddress, string memory tokenURI, uint256 expirationSeconds)
        public
        returns (uint256)
    {
        ...
        tokenDetails[newItemId] = TokenDetails({
            tokenURI: tokenURI,
            owner: useraddress,
            expirationTime: block.timestamp + expirationSeconds // Set expiration time in seconds
        });
        ...
        returns (TokenDetails[] memory)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (block.timestamp < details.expirationTime) {
                // Token has not expired, include it in the result
                result[i] = details;
            } else {
                // Token has expired, set empty string for URI
                result[i].tokenURI = "";
                result[i].owner = details.owner;
                result[i].expirationTime = details.expirationTime;
            }
        }
